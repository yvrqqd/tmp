...
st.write("Также ошибки, которые иногда выдаются, можно отслеживать с помощью агентов.")
st.write("Еще можно иметь базу городов и искать в ней ответ, данный моделью. Таким образом можно однозначно ограничить простор для творчества модели и проверять нужно будет только на корректность определения города, а уже в этом полагаться на агентов.")
